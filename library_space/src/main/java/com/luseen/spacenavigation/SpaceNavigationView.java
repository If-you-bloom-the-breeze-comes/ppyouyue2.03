...
public class SpaceNavigationView extends RelativeLayout {
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            activeSpaceItemColor = typedArray.getColor(com.luseen.spacenavigation.R.styleable.SpaceNavigationView_active_item_color, resources.getColor(R.color.app_green));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (centreButtonIcon == NOT_DEFINED)
            centreButtonIcon = R.drawable.yue_shadow;
    ...
    protected void onSizeChanged(int width, int height, int oldWidth, int oldHeight) {
        ...
        //update by mayue 2017.12.27 不显示中间按钮
...
//        centreButton.setImageResource(centreButtonIcon);
        centreButton.setBackgroundResource(R.drawable.yue_shadow);
//
...
//            centreButton.getDrawable().setColorFilter(inActiveCentreButtonIconColor, PorterDuff.Mode.SRC_IN);
        ...
        mainContentParams.addRule(RelativeLayout.ALIGN_BOTTOM);

        /**
         * Centre content size
         */
        LayoutParams centreContentParams = new LayoutParams(centreContentWight, spaceNavigationHeight-50);
       centreContentParams.addRule(RelativeLayout.CENTER_HORIZONTAL);
       centreContentParams.addRule(RelativeLayout.ALIGN_BOTTOM);
        centreContentParams.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);

        /**
         * Centre Background View content size and position
         */
        LayoutParams centreBackgroundViewParams = new LayoutParams(spaceNavigationHeight, mainContentHeight);
    ...
    private void addSpaceItems(LinearLayout leftContent, LinearLayout rightContent) {
        ...
        for (int i = 0; i < spaceItems.size(); i++) {
            ...
            if(i==0){
                spaceItemIcon.setBackgroundResource(spaceItems.get(i).getItemIcomFouced());
            }else{
                spaceItemIcon.setBackgroundResource(spaceItems.get(i).getItemIcon());
            }
...
//                spaceItemText.setTextColor(activeSpaceItemColor);
                Utils.changeImageViewTint(i,spaceItemIcon, activeSpaceItemColor);
            } else {
//                spaceItemText.setTextColor(inActiveSpaceItemColor);
                Utils.changeImageViewTint(i,spaceItemIcon, inActiveSpaceItemColor);
            }
            ...
            if (restoredBundle.containsKey(CHANGED_ICON_AND_TEXT_BUNDLE_KEY)) {
                ...
                if (changedItemAndIconHashMap != null) {
                    ...
                    for (int i = 0; i < changedItemAndIconHashMap.size(); i++) {
                        ...
                        spaceItems.get(i).setItemIcomFouced(spaceItem.getItemIcomFouced());
                    }
                }
            }
...
//            centreButtonIcon = icon;
    ...
    public void changeItemIconAtPosition(int itemIndex, int newIcon) {
        if (itemIndex < 0 || itemIndex > spaceItems.size()) {
            ...
            ImageView spaceItemIcon = (ImageView) textAndIconContainer.findViewById(R.id.space_icon);
            spaceItemIcon.setBackgroundResource(newIcon);
     ...
     * @param color target color
     */
    public void setCurrentSelectedItem1( int color) {
